...
Node *parse(Token *tail, int startLayer, int maxLayer, int until)
{
    ...
    // for (int curLayer = 0; curLayer < 1; curLayer++)
    {
        ...
        while (curr != NULL && curr->number >= until)
        {
            ...
            //     printf("Error! Nothing to parse %s\n","");
            //     return NULL;
            // }
                        ...
                        open->layer == curLayer)
                    {
                        ...
                        if (open->prev != NULL && open->prev->this->type == BIN)
                        {
                            ...
                            while (open != NULL && open->number < curr->number)
                            {
                                ...
                                // printf("type name :$%d$ $%d$ open layer: %d, curLayer: %d\n", open->this->type == COM, strcmp(open->this->name, ",") == 0, open->layer, curLayer);

                                // Check for the comma
                                if (open->this->type == COM &&
                                    // This strcmp is unnecessary
...
long calculate(Node *root, Variable *first)
{
    ...
    // printf("destructure line 625 %s\n", "");

    // VARIABLE
    if (root->type == VAR)
    {
        // printf("destructure line 630 %s\n", "");

        return getVariable(first, root->name);
    }
    // printf("destructure line 634 %s\n", "");
